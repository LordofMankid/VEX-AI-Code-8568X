...
const double TR_INCH_TICK = TICKS_TRACKING_REV/(2.75*PI);

void odometry::updatePosition(){
//steps will correspond with Pilons thing

  //step 1 - update tracking wheels, convert to inches
  positionL = trackingLeft.value()*TR_TICK_INCH;
  positionR = trackingRight.value()*TR_TICK_INCH;
  positionH = trackingX.value()*TR_TICK_INCH;
